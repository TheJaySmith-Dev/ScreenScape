...
export const BrandDetail: React.FC<BrandDetailProps> = ({
    ...
    return (
        <>
            {brand.backdropUrl && (
                <div 
                    className="fixed inset-0 w-full h-full z-[-1] transition-all duration-500 ease-in-out"
                >
                    <img 
            ...
            <div className="relative z-10 w-full max-w-7xl fade-in text-white">
                <div className="flex items-center gap-4 mb-6">
                    <button onClick={onBack} className="px-4 py-2 text-sm glass-panel text-white rounded-full hover:bg-white/20 transition-colors">&larr; Back to Brands</button>
                    <h2 className="text-2xl font-bold" style={{textShadow: '0 2px 4px rgba(0,0,0,0.2)'}}>{brand.name}</h2>
                </div>

                {brand.characterCollections.length > 0 && (
                    <section className="w-full mb-8 md:mb-12">
                      <h3 className="text-xl md:text-2xl font-bold mb-4 ml-4 md:ml-0" style={{textShadow: '0 2px 4px rgba(0,0,0,0.2)'}}>Character Collections</h3>
                ...
                <h3 className="text-xl md:text-2xl font-bold mb-4" style={{textShadow: '0 2px 4px rgba(0,0,0,0.2)'}}>All {brand.name} Content</h3>
                <StudioFilters
                  mediaTypeFilter={mediaTypeFilter}
